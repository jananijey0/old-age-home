...
?>
<form name="f" action="adminhome.php" method="post" onsubmit="return validate_newperson()" enctype="multipart/form-data">
<table class="center_tab">
    ...
    <tbody>
        ...
        <td><textarea name="hproblem" cols="24" rows="3" required></textarea></td>
        </tr>
        <tr>
            <th>Aadhar No</th>
            <td><input type="text" name="aadhar" maxlength="12" required></td>
        </tr>
        <tr>
            <th>Photo</th>
...
<?php
    ...
    if(is_uploaded_file($_FILES['ff']['tmp_name'])) {
        ...
        move_uploaded_file($_FILES['ff']['tmp_name'], $fname) or die("Photo cannot be Moved...!");
        $dt = date('Y-m-d',time());
    mysqli_query($link, "insert into newperson(name,gender,age,addr,city,occupation,pension,relative,relation,mobile,reason,joindt,hproblem,aadhar,fname) values ('$name','$gender','$age','$addr','$city','$occupation','$pension','$relative','$relation','$mobile','$reason','$dt','$hproblem','$aadhar','$fname')");
    echo "<div class='center'>New Person Information Stored...!<br><a href='adminhome.php'>Back</a></div>";
    } else {
        echo "<div class='center'>Photo Not Uploaded...!<br><a href='adminhome.php'>Back</a></div>";
    }    
}
include './footer.php';
?>